"""Train OF-SATNet on Feishu A-share equity data.
...
from models.ofsatnet import OFSATNet, count_parameters
...
def main() -> None:
    ...
    parser.add_argument(
        "config", nargs="?", default="configs/stocks/feishu/ofsatnet_ofi.json"
    )
    ...
    ckpt_dir = (
        ...
        / f"ofsatnet_{config.get('feature_mode', 'ofi')}_{stamp}"
    )
    ...
    logger.info(
        "OF-SATNet [Feishu]  mode={}  symbols={}  n_features={}",
        config.get("feature_mode"),
        len(symbols),
        config["n_features"],
    )
    logger.info(
        "  params={:.2f}M  device={}", count_parameters(OFSATNet(config)) / 1e6, device
    )
    ...
    model = OFSATNet(config).to(device)
