"""Train AxialLOB on Feishu A-share equity data.

Usage::

    uv run python -m stocks.feishu_midprice.train_axiallob configs/stocks/feishu_midprice/axiallob_h1.json
"""
...
from models.axiallob import AxialLOB, count_parameters
...
def main() -> None:
    ...
    parser.add_argument(
        "config", nargs="?", default="configs/stocks/feishu_midprice/axiallob_h1.json"
    )
    ...
    ckpt_dir = (
        ...
        / f"axiallob_{config.get('feature_mode', 'ofi')}_{stamp}"
    )
    ...
    logger.info(
        "AxialLOB [Feishu]  mode={}  symbols={}  n_features={}",
        config.get("feature_mode"),
        len(symbols),
        config["n_features"],
    )
    logger.info(
        "  params={:.2f}M  device={}", count_parameters(AxialLOB(config)) / 1e6, device
    )
    ...
    model = AxialLOB(config).to(device)
