"""Train TLOB on Feishu A-share equity data.

Usage::

    uv run python -m stocks.feishu_midprice.train_tlob configs/stocks/feishu_midprice/tlob_h1.json
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "config", nargs="?", default="configs/stocks/feishu_midprice/tlob_h1.json"
    )
    ...
    ckpt_dir = (
        ...
        / f"tlob_{config.get('feature_mode', 'ofi')}_{stamp}"
    )
    ...
    logger.info(
        "TLOB [Feishu]  mode={}  symbols={}  n_features={}",
        config.get("feature_mode"),
        len(symbols),
        config["n_features"],
    )
    logger.info(
        "  params={:.2f}M  dim={}  blocks={}  heads={}  device={}",
