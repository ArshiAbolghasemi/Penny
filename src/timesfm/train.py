"""Training entry point for the TimesFM price-direction forecaster.
...
from .model import TimesFMForecastModel, TrendHead, count_parameters
...
def validate_label_accuracy(model, loader, config, alpha, device):
    ...
    for batch in loader:
        ...
        for lv, true in zip(l_pred, batch["label"].numpy()):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Penny TimesFM forecaster.")
    ...
    ckpt_dir = (
        Path(config["checkpoint_root"]) / f"timesfm_{config['feature_mode']}_{stamp}"
    )
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    logger.add(ckpt_dir / "train.log", level="DEBUG")

    train_ds, val_ds, test_ds, alpha, meta = build_datasets(config)
    model = TimesFMForecastModel(config).to(device)
    trend_head = TrendHead().to(device)
    cb = meta["class_balance"]
    logger.info(
        "TimesFM forecaster — feature_mode={} (mid series)", config["feature_mode"]
    )
    logger.info(
