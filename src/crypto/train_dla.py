"""Train DLA on a single crypto symbol.

Usage::

    uv run python -m crypto.train_dla configs/crypto/binance/dla/btcusdt_ofi.json
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "config", nargs="?", default="configs/crypto/binance/dla/btcusdt_ofi.json"
    )
    ...
    ckpt_dir = (
        ...
        / f"dla_{config['symbol']}_{config.get('feature_mode', '')}_{stamp}"
    )
    ...
    logger.info("DLA  symbol={}  mode={}", config["symbol"], config.get("feature_mode"))
    ...
    logger.info(
        ...
        meta["n_features"],
        count_parameters(DLA(config)) / 1e6,
    )
