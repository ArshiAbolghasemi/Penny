"""Train AxialLOB on a single crypto symbol.

Usage::

    uv run python -m crypto.train_axiallob configs/crypto/binance/axiallob/btcusdt_ofi.json
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "config", nargs="?", default="configs/crypto/binance/axiallob/btcusdt_ofi.json"
    )
    ...
    ckpt_dir = (
        ...
        / f"axiallob_{config['symbol']}_{config.get('feature_mode', '')}_{stamp}"
    )
    ...
    logger.info(
        "AxialLOB  symbol={}  mode={}", config["symbol"], config.get("feature_mode")
    )
    ...
    logger.info(
        ...
        meta["n_features"],
        count_parameters(AxialLOB(config)) / 1e6,
    )
