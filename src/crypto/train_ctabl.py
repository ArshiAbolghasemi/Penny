"""Train CTABL on a single crypto symbol.

Usage::

    uv run python -m crypto.train_ctabl configs/crypto/binance/ctabl/btcusdt_ofi.json
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "config", nargs="?", default="configs/crypto/binance/ctabl/btcusdt_ofi.json"
    )
    ...
    ckpt_dir = (
        ...
        / f"ctabl_{config['symbol']}_{config.get('feature_mode', '')}_{stamp}"
    )
    ...
    logger.info(
        "CTABL  symbol={}  mode={}", config["symbol"], config.get("feature_mode")
    )
    ...
    logger.info(
        ...
        meta["n_features"],
        count_parameters(CTABL(config)) / 1e6,
    )

    model = CTABL(config).to(device)
