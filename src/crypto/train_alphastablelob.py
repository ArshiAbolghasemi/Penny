...
Same joint structure as ``crypto.train_jumpgatelob`` — a shared trunk trained on
...
**L_robust** (the same noise-consistency term ``crypto.train_jumpgatelob`` uses, here
...
def main() -> None:
    ...
    ckpt_dir = (
        ...
        / f"alphastablelob_{mode}_a{alpha}_{config['symbol']}"
        f"_{config.get('feature_mode', '')}_{stamp}"
    )
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    logger.add(ckpt_dir / "train.log", level="DEBUG")

    train_ds, val_ds, test_ds, alpha_thr, meta = build_datasets(config)
    config["n_features"] = meta["n_features"]
    cb = meta["class_balance"]

    logger.info(
        "AlphaStableLOB  symbol={} mode={} alpha={}", config["symbol"], mode, alpha
    )
