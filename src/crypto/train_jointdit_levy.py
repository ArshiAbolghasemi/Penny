"""Train JointDiT-Lévy: joint Lévy jump-diffusion + trend classifier (DiT backbone).

Identical objective and forward process to ``crypto.train_jointdiff_levy`` but the
U-Net backbone is replaced by the **DiT** (Diffusion Transformer, ``models.jointdit``),
which already provides adaLN-Zero timestep conditioning and a feature-only
classification head — so the model is reused unchanged and only the *interpretation*
of its output changes: the un-patchified map is the predicted **generalized score**
rather than epsilon.
...
Joint objective (Deja et al. 2023) on the shared DiT tokens:
...
Inference is feature-only: ``model.predict`` runs the DiT + trend head on the clean
window at t=0 (no un-patchify sampling loop needed for the trend label).
...
def main() -> None:
    ...
    logger.info(
        "JointDiTLevy  symbol={}  mode={}  process={}  schedule={}",
        config["symbol"],
    ...
    logger.info(
        "  params={:.2f}M  gflops/sample={:.3f}  dim={}  depth={}  jump_rate={}  device={}",
        count_parameters(model) / 1e6,
        gflops,
        config.get("jdit_dim", 192),
        config.get("jdit_depth", 6),
