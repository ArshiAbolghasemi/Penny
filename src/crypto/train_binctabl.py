"""Train BINCTABL on a single crypto symbol.

Usage::

    uv run python -m crypto.train_binctabl configs/crypto/binance/binctabl/btcusdt_ofi.json
"""
...
from models.binctabl import BINCTABL, count_parameters
...
def main() -> None:
    ...
    parser.add_argument(
        "config", nargs="?", default="configs/crypto/binance/binctabl/btcusdt_ofi.json"
    )
    ...
    ckpt_dir = (
        ...
        / f"binctabl_{config['symbol']}_{config.get('feature_mode', '')}_{stamp}"
    )
    ...
    logger.info(
        "BINCTABL  symbol={}  mode={}", config["symbol"], config.get("feature_mode")
    )
    ...
    logger.info(
        ...
        meta["n_features"],
        count_parameters(BINCTABL(config)) / 1e6,
    )

    model = BINCTABL(config).to(device)
