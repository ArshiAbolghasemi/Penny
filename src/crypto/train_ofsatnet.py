"""Train OF-SATNet on a single crypto symbol.
...
def main() -> None:
    ...
    ckpt_dir = (
        ...
        / f"ofsatnet_{config['symbol']}_{config.get('feature_mode', '')}_{stamp}"
    )
    ...
    logger.info(
        "OF-SATNet  symbol={}  mode={}", config["symbol"], config.get("feature_mode")
    )
    ...
    logger.info(
        ...
        meta["n_features"],
        count_parameters(OFSATNet(config)) / 1e6,
    )
