"""Train a linear-recurrent VAR-style on crypto LOB data.

A trainable *linear* classifier that uses the exact same dataset, training protocol
(CE + AdamW + warmup→cosine, early stopping on val CE) and ``stride=1`` evaluation as
the neural models — so its accuracy / macro-F1 are directly comparable to DeepLOB,
CTABL, JumpGateLOB, etc. (unlike the per-point-refit ARIMA/VAR statistical baselines).
...
def main() -> None:
    ...
    parser.add_argument(
        "config", nargs="?", default="configs/crypto/coinbase/linvar/btcirt_ofi_k10.json"
    )
    ...
    ckpt_dir = Path(config["checkpoint_dir"]) / f"linvar_{config['symbol']}_{stamp}"
    ...
    logger.info(
        "LinVAR  symbol={}  mode={}", config["symbol"], config.get("feature_mode")
    )
    ...
    logger.info(
        ...
        meta["n_features"],
        count_parameters(LinVAR(config)) / 1e6,
    )

    model = LinVAR(config).to(device)
