"""Train a multinomial logistic-regression baseline on crypto LOB data.
...
def main() -> None:
    ...
    parser.add_argument(
        "config",
        nargs="?",
        default="configs/crypto/coinbase/logreg/btcirt_ofi_k10.json",
    )
    ...
    logger.info(
        "LogReg  symbol={}  mode={}", config["symbol"], config.get("feature_mode")
    )
    ...
    logger.info(
        ...
        meta["n_features"],
        count_parameters(LogReg(config)) / 1e6,
    )
