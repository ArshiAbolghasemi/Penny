...
ofsatnet_levels        number of order-book levels L forming the Level path
                        (must be <= n_features; the first L feature columns are
                        taken as the per-level OFI slice)              (default 10)
...
class OFSATNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        levels = x[:, :, : self.L].transpose(1, 2)  # (B, L, T) — tokens = per-level series
        h_l = self.level(levels)  # level path: attends over L order-book levels
