...
def test_set(k: int):
    ...
    _feat, mid, ts = build_cache(cfg, extract_features, n_features, tag="lob")
...
from crypto.dataset import LOBDataset, _valid_starts
from crypto.labels import build_labels
from crypto.loader import _rolling_mean_std
...
def _corrupt_test_rows(kind, sigma):
    ...
    seed = ROBUST_SEED + 100_003 * ROBUST_KINDS.index(kind) + round(10_000 * sigma)
    rng = np.random.default_rng(seed)
...
def run_raw_robust_sweep():
    ...
    for k in HORIZONS:
        cases = {}
        clean_ds, clean_y, _clean_meta = raw_test_set(k, "gaussian", 0.0)
        ...
        if ROBUST_MAX_WINDOWS is None and not np.array_equal(clean_y, reference_y):
        ...
        for kind in ROBUST_KINDS:
            for sigma in ROBUST_SIGMAS:
                ds, y, _meta = raw_test_set(k, kind, sigma)
